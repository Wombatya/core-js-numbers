...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  const squaredDifferenceSum = xDifference ** 2 + yDifference ** 2;
...
function roundToPowerOfTen(num, pow) {
  ...
  if (arrNum[arrNum.length - pow] >= 5) {
    arrNum[arrNum.length - 1 - pow] = +arrNum[arrNum.length - 1 - pow] + 1;
    const str = arrNum.join('');
    return +`${str.slice(0, -pow)}${'0'.repeat(pow)}`;
  }
  return +`${num.toString().slice(0, -pow)}${'0'.repeat(pow)}`;
}
...
function getSumOfDigits(num) {
  ...
  return arr.map((el) => +el).reduce((acc, el) => acc + el);
}
...
function getSumOfNumbers(x1, x2, x3) {
  const result = Array.from([x1, x2, x3]).reduce((acc, el) => acc + el);
